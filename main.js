	...
	if (message.isMemberMentioned(client.user) && message.attachments.size > 0) {
		...
		if (message.attachments.every(attachIsImage)) {
			message.attachments.forEach(function(attachment) {
				// メッセージにコマンド含む場合
				bot_message += /l|L/gi.test(message.content) ? 'L武器のみで解析を開始します。' : '全武器で解析を開始します。';
				...
				// 表示形式フラグ（一旦falseに）
				if (/s|summary|さまり|サマリ|がいよう|概要|ぶき|武器|c|C|ころ|コロ|h|補助|ほじょ|ほ/gi.test(message.content)) {
					summary_flag = false;
					col_skill_flag = false;
					col_aid_skill_flag = false;
				}
				if (/s|summary|さまり|サマリ|がいよう|概要|ぶき|武器/gi.test(message.content)) {
					summary_flag = true;
				}
